...
object LocalAiEngine {
    ...
    fun init(context: Context): Boolean {
        ...
        if (!LlamaJniWrapper.isAvailable()) {
            Log.w(TAG, "Native library not available — local AI disabled")
            return false
        }
    ...
    fun releaseModel(context: Context) {
        if (modelHandle != 0L && LlamaJniWrapper.isAvailable()) {
    ...
    fun getLogs(): String = if (LlamaJniWrapper.isAvailable()) LlamaJniWrapper.getLogs() else ""

    /** Clear the log buffer */
    fun clearLogs() {
        if (LlamaJniWrapper.isAvailable()) LlamaJniWrapper.clearLogs()
    }
