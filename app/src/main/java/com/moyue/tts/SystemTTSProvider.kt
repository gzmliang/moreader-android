...
class SystemTTSProvider(context: Context) : TTSProvider {
    ...
    private fun ensureInitialized() {
        ...
        // ⚠️ Engine already alive and ready — return immediately (Pitfall: avoid
        // calling setupTts/setLanguage on every speak, which disables onRangeStart!)
        if (globalTts != null && currentEngine == systemEngine && globalIsReady) return
        // Engine exists but not marked ready — just mark it
        if (globalTts != null && currentEngine == systemEngine) { globalIsReady = true; return }
            ...
            try {
                globalTts = TextToSpeech(ctx, { status ->
                    ...
                    if (status == TextToSpeech.SUCCESS) {
                        ...
                        // Skip setupTts on incomplete onInit (voices=0, double-init bug)
                        if (tts != null && (tts?.voices?.size ?: 0) > 0) setupTts(tts)
    ...
    private fun setupTts(tts: TextToSpeech) {
        try {
            ...
            dlog("Listener 设置完成")
            // ⚠️ setLanguage MUST come AFTER listener! (Pitfall #5.3)
            // setLanguage can reset onRangeStart in some engines.
            tts.setLanguage(Locale.CHINESE)
            cachedLocale = Locale.CHINESE  // sync cache to avoid redundant setLanguage
            dlog("中文设置完成")
        } catch (e: Exception) {
    ...
    private fun speakWithRetry(text: String, rate: Float, listener: TTSListener, retryCount: Int) {
        ...
        // Uses cached locale to avoid calling setLanguage on every utterance.
